...
| Crazy Studio | Creative prototyping stack, campaign experimentation tools | `stitch`, `runcomfy`, `fetch`, `sequential-thinking` | `mcp-playwright` |
| Theatrical Agency | Stage-inspired creative production stack, immersive launch tooling, run-of-show orchestration | `runcomfy`, `stitch`, `fetch`, `mcp-playwright`, `lighthouse` | `sequential-thinking`, `ios-simulator`, `snyk`, `supabase` |
| Wolf of Wall Crypto Agency | Web stack + analytics + crypto integration stack | `fetch`, `supabase`, `snyk`, `mcp-playwright`, `revenuecat` | `lighthouse`, `stitch`, `runcomfy`, `sequential-thinking` |
...
- Keep this matrix aligned with `.agents/office-config.md`, `.agents/agencies/`, and `.agents/agents/`.
