...
1. **Read advance_mode**: read `.ai-office/project.config.md` and extract `advance_mode` and `agency`. Default advance_mode: `manual`.

2. **Load pipeline** (custom or default):
   - Check if `.ai-office/agencies/<agency>/pipeline.md` exists.
   - If it exists: read the `## Transition Rules` table and `## Loop Guards` table from that file. Use these **instead of** the hardcoded tables above.
   - If it does not exist: use the default Stage → Agent Mapping and Pipeline Transitions tables above.

3. **Read current state**: read `.ai-office/docs/runbooks/<slug>-status.md` to get current state. If the file does not exist, stop and output:
   ```
...
4. **Determine the next stage**: use the `[next-stage]` argument if provided, otherwise use the transition table (custom or default from step 2).

5. **Check and increment loop guards**:

   Read the `## Loop Guards` table from the status file. If the table is absent, treat all counters as 0 with their default maxes from the pipeline (custom or default).

   Default guards (used when no custom pipeline):
   ...
   If a custom pipeline is loaded, use its `## Loop Guards` table instead.
...
6. **In `manual` mode**: before making changes, output a summary:
   ```
   Advancing: <slug>
   <old-state> → <new-stage>
   Evidence: <evidence>
   Agent: <responsible agent for new stage>
   Pipeline: custom (<agency>) / default
   ```
...
8. **Reassign tasks**: scan `.ai-office/tasks/` (BACKLOG, TODO, WIP, REVIEW, BLOCKED columns — not DONE or ARCHIVED) for tasks belonging to this slug. A task belongs to this slug if **any** of the following match:
...
10. **Next action**: show the recommended action for the new stage:
    ...
    - Custom stage → "Check `.ai-office/agents/<agent>/personality.md` for role guidance"
